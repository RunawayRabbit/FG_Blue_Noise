...
public static class MathfsExtensions
{
	...
	public static Vector2 RotateAround( this Vector2 v,
										Vector2      pivot,
										float        angRad ) =>
	...
	public static Vector3 XZtoXYZ( this Vector2 v, float y = 0 ) =>
	...
	public static Vector2 Clamp01( this Vector2 v ) =>
		new Vector2( Mathfs.Clamp01( v.x ), Mathfs.Clamp01( v.y ) );

	public static Vector3 Clamp01( this Vector3 v ) =>
		new Vector3( Mathfs.Clamp01( v.x ),
					 Mathfs.Clamp01( v.y ),
					 Mathfs.Clamp01( v.z ) );

	public static Vector4 Clamp01( this Vector4 v ) =>
		new Vector4( Mathfs.Clamp01( v.x ),
					 Mathfs.Clamp01( v.y ),
					 Mathfs.Clamp01( v.z ),
					 Mathfs.Clamp01( v.w ) );
	...
	public static float Max( this Vector4 v ) =>
		Mathfs.Max( v.x, v.y, v.z, v.w );

	public static Vector2 WithMagnitude( this Vector2 v, float mag ) =>
		v.normalized * mag;

	public static Vector3 WithMagnitude( this Vector3 v, float mag ) =>
		v.normalized * mag;
	...
	public static Color MultiplyRGB( this Color c, Color m ) =>
		new Color( c.r * m.r, c.g * m.g, c.b * m.b, c.a );

	public static Color MultiplyA( this Color c, float m ) =>
		new Color( c.r, c.g, c.b, c.a * m );

	public static float Round( this float v, float snapInterval ) =>
		Mathfs.Round( v, snapInterval );

	public static bool Within( this float v, float min, float max ) =>
		v >= min && v <= max;

	public static bool Between( this float v, float min, float max ) =>
		v > min && v < max;
	...
	public static float Remap( this float v,
							   float      iMin,
							   float      iMax,
							   float      oMin,
							   float      oMax ) =>
		Mathfs.Remap( iMin, iMax, oMin, oMax, v );

	public static float Repeat( this float v, float length ) =>
		Mathfs.Repeat( v, length );
