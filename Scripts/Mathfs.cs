...
// used for arbitrary count min/max functions, so it's safe and won't allocate garbage don't worry~
...
public static class Mathfs
{
	...
	public static bool Approximately( float a, float b ) =>
		Abs( b - a )
		< Max( 0.000001f * Max( Abs( a ), Abs( b ) ), Epsilon * 8 );
	...
	public static Vector2 Abs( Vector2 v ) =>
		new Vector2( Abs( v.x ), Abs( v.y ) );

	public static Vector3 Abs( Vector3 v ) =>
		new Vector3( Abs( v.x ), Abs( v.y ), Abs( v.z ) );

	public static Vector4 Abs( Vector4 v ) =>
		new Vector4( Abs( v.x ), Abs( v.y ), Abs( v.z ), Abs( v.w ) );
	...
	public static Vector2 Floor( Vector2 value ) =>
		new Vector2( (float) Math.Floor( value.x ),
					 (float) Math.Floor( value.y ) );

	public static Vector3 Floor( Vector3 value ) =>
		new Vector3( (float) Math.Floor( value.x ),
					 (float) Math.Floor( value.y ),
					 (float) Math.Floor( value.z ) );

	public static Vector4 Floor( Vector4 value ) =>
		new Vector4( (float) Math.Floor( value.x ),
					 (float) Math.Floor( value.y ),
					 (float) Math.Floor( value.z ),
					 (float) Math.Floor( value.w ) );
	...
	public static float Round( float value, float snapInterval ) =>
	...
	public static int Mod( int value, int length ) =>
		(value % length + length) % length; // modulo
	...
	public static float Smoother01( float x ) =>
		x * x * x * (x * (x * 6 - 15) + 10);
	...
	public static Vector2 Lerp( Vector2 a, Vector2 b, Vector2 t ) =>
		new Vector2( Mathf.Lerp( a.x, b.x, t.x ), Mathf.Lerp( a.y, b.y, t.y ) );
	...
	public static Vector2 Remap( Rect iRect, Rect oRect, Vector2 iPos ) =>
		Remap( iRect.min, iRect.max, oRect.min, oRect.max, iPos );

	public static Vector2 Remap( Vector2 iMin,
								 Vector2 iMax,
								 Vector2 oMin,
								 Vector2 oMax,
								 Vector2 value )
	...
	public static float Remap( float iMin,
							   float iMax,
							   float oMin,
							   float oMax,
							   float value )
	...
	public static float RemapClamped( float iMin,
									  float iMax,
									  float oMin,
									  float oMax,
									  float value )
	...
	public static float MoveTowards( float current,
									 float target,
									 float maxDelta )
	{
	...
	public static float Determinant /*or Cross*/( Vector2 a, Vector2 b ) =>
		a.x * b.y - a.y * b.x; // 2D "cross product"
	...
	public static Vector2 CenterDir( Vector2 aDir, Vector2 bDir ) =>
		(aDir + bDir).normalized;

	public static Vector3 CenterDir( Vector3 aDir, Vector3 bDir ) =>
		(aDir + bDir).normalized;
	...
	public static Vector2 AngToDir( float aRad ) =>
		new Vector2( Mathf.Cos( aRad ), Mathf.Sin( aRad ) );

	public static float DirToAng( Vector2 dir ) => Mathf.Atan2( dir.y, dir.x );

	public static float SignedAngle( Vector2 a, Vector2 b ) =>
		AngleBetween( a, b )
		* Mathf.Sign( Determinant( a, b ) ); // -tau/2 to tau/2

	public static float AngleBetween( Vector2 a, Vector2 b ) =>
		Mathf.Acos( Vector2.Dot( a.normalized, b.normalized ) );

	public static float AngleFromToCW( Vector2 from, Vector2 to ) =>
		Determinant( from, to ) < 0 ? AngleBetween( from, to ) :
	...
	public static float MoveTowardsAngleDeg( float current,
											 float target,
											 float maxDelta )
	{
		float deltaAngle = DeltaAngleDeg( current, target );

		if( -maxDelta < deltaAngle
			&& deltaAngle < maxDelta )
			return target;
	...
	public static float SmoothDampAngleDeg( float     current,
											float     target,
											ref float currentVelocity,
											float     smoothTime,
											float     maxSpeed )
	{
		...
		return SmoothDampAngleDeg( current,
								   target,
								   ref currentVelocity,
								   smoothTime,
								   maxSpeed,
	...
	public static float DeltaAngleDeg( float a, float b ) =>
		(b - a + 180).Repeat( 360 ) - 180;

	private static float InverseLerpAngleDeg( float a, float b, float v )
	{
	...
	public static float MoveTowardsAngleRad( float current,
											 float target,
											 float maxDelta )
	{
	...
	public static float SmoothDampAngleRad( float     current,
											float     target,
											ref float currentVelocity,
											float     smoothTime,
											float     maxSpeed )
	{
		...
		return SmoothDampAngleRad( current,
								   target,
								   ref currentVelocity,
								   smoothTime,
								   maxSpeed,
	...
	public static bool TryGetLinearRoot( float k, float m, out float root )
	{
		// kx + m
	...
	public static PolynomialType GetPolynomialType( float a, float b, float c )
	{
		if( Mathf.Abs( a ) < 0.00001f )
			return Mathf.Abs( b ) < 0.00001f ? PolynomialType.Constant :
		...
		switch( GetPolynomialType( a, b, c ) )
		{
			...
			case PolynomialType.Quadratic:
				...
				if( Mathf.Abs( rootContent ) < 0.0001f )
					roots.Add(
						-b / (2 * a) ); // two equivalent solutions at one point
	...
	public static Vector2 DiscToSquare( Vector2 c )
	{
		...
		var         n       = new Vector2( 1, -1 );
		Vector2     p       = new Vector2( 2, 2 ) + n * (u2 - v2);
		Vector2     q       = 2 * SQ2 * c;
		Vector2     smolVec = Vector2.one * 0.0001f;

		Vector2 Sqrt( Vector2 noot ) =>
			new Vector2( Mathf.Sqrt( noot.x ), Mathf.Sqrt( noot.y ) );

		return 0.5f
			   * (Sqrt( Vector2.Max( smolVec, p + q ) )
				  - Sqrt( Vector2.Max( smolVec, p - q ) ));
	}
	...
	public static class Triangle
	{
		public static float SignedArea( Vector3 a, Vector3 b, Vector3 c ) =>
			Vector3.Cross( b - a, c - a ).magnitude / 2f;

		public static float Area( Vector3 a, Vector3 b, Vector3 c ) =>
			Mathf.Abs( SignedArea( a, b, c ) );
		...
		public static (Vector2 center, float radius) Circumcircle(
			Vector2 a,
			Vector2 b,
			Vector2 c )
		{
			Vector2 SqSub( Vector2 p0, Vector2 p1 ) =>
				Vector2.Scale( p0, p0 ) - Vector2.Scale( p1, p1 );
			...
			Vector2 center = new Vector2( noot.y, -noot.x )
							 / (2 * Determinant( CA, BA ));
		...
		public static bool Contains( Vector2 a,
									 Vector2 b,
									 Vector2 c,
									 Vector2 point,
									 float   aMargin = 0f,
									 ...
									 float   cMargin = 0f )
		{
			...
			return
				b0 == b1
				&& b1
				== b2; // on the same side of all halfspaces, this can only happen inside
		}
	}

	public static class Polygon
	{
		public static bool IsClockwise( IReadOnlyList<Vector2> pts ) =>
			SignedArea( pts ) > 0;

		public static float Area( IReadOnlyList<Vector2> pts ) =>
			Mathf.Abs( SignedArea( pts ) );
	...
	{
		/// <summary>
		///     Outputs the launch speed required to traverse a given lateral distance when launched at a given angle, if one
		///     exists
		/// </summary>
		...
		public static bool TryGetLaunchSpeed( float     gravity,
											  ...
											  out float speed )
		{
			...
			if( Mathf.Abs( den ) < 0.00001f )
			{
				...
				return
					false; // direction is parallel, no speed would get you there
			}
			...
			if( speedSquared < 0 )
			{
				...
				return
					false; // can't reach destination because you're going the wrong way
			}
		...
		public static bool TryGetLaunchAngles( float     gravity,
											   float     lateralDistance,
											   float     speed,
											   out float angleLow,
											   out float angleHigh )
		...
		public static Vector2 GetDisplacement( float gravity,
											   float speed,
											   float angle,
											   float time )
		{
			...
			float yDisp = speed * time * Mathf.Sin( angle )
						  - .5f * gravity * time * time;

			return new Vector2( xDisp, yDisp );
		}

		/// <summary>
		///     Returns the maximum height that can possibly be reached if speed was redirected upwards, given a current height and
		///     speed
		/// </summary>
		/// <param name="gravity">Gravitational acceleration in meters per second</param>
		...
		public static float GetHeightPotential(
			float gravity,
			float currentHeight,
			float speed ) =>
		...
		public static bool TryGetSpeedFromHeightPotential(
			float     gravity,
			float     currentHeight,
